...
parser.add_argument('--lr', type=float, default=2.8e-4, help='学习率')
...
parser.add_argument('--pseudo_ratio_neg', type=float, default=0.45, help='正常样本权重')
parser.add_argument('--pseudo_ratio_pos', type=float, default=0.3, help='异常样本权重')
parser.add_argument('--pseudo', type=str, default="result.csv", help='伪标签文件')
...
def read_pickle(file, f, pseudo=0, output_features=None):
    """
    读取训练集和测试集数据
    """
    if f.endswith(".pkl"):
        ...
        # if output_features:
        #     output_feature = output_features.get(f, [0., 0.])
        #     # data["output_feature_0"] = output_feature[0]
        #     data["output_feature_1"] = output_feature[1]
...
# 读取模型输出的 output
# output_features = load_pickle("cache/output_features.pkl")
...
# 加入伪标签数据
bad_index = int(sum([d["label"][0] for d in train]) / len(train) * args.pseudo_ratio_pos * len(test))
good_index = int((len(train) - sum([d["label"][0] for d in train])) / len(train) * args.pseudo_ratio_neg * len(test))

pred_score = pd.read_csv(args.pseudo).sort_values("score").reset_index(drop=True)
...
if good_index > 0:
    pseudo = pseudo + [read_pickle("Test_A", f, pseudo=0) for f in tqdm(good_filename, desc="load pseudo good :::")]
train.extend(pseudo)
...
# 模型保存
torch.save(model, "vloong_anomaly_detection_model_pseudo.torch")
...
# 生成结果文件
pd.DataFrame(list(zip(test_index, pred)), columns=["file_name", "score"]).to_csv("pseudo_result.csv", index=False)
